	...
	<div class="row g-3 mb-3 align-items-center">
		<div class="col">
			<ol class="breadcrumb bg-transparent mb-0">
				...
				<li class="breadcrumb-item"> <a class="text-secondary" href="{{url('service')}}" aria-current="page">Service</a></li>
		...
		<div class="row g-4">
			<div class="col-lg-12 col-md-12 col-sm-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="{{route('service.update',$services->id)}}" method="post" id="form" enctype="multipart/form-data">
							@csrf
							@method('PUT')
							<div class="form-group col-md-6 mb-3">
								<label> Service Name </label>
								<input type="text" value="{{$services->service_name}}" class="form-control" name="service_name" id="service_name_id" placeholder="Service Name" required>
							</div>
							<div class=" form-group col-md-6">
								<div id="delete-container{{$services->id}}">
									<label>Icon</label>
									<input type="file" style="width:480px" name="edit_service_image[]" class="form-control update-image dropify" data-default-file="{{ asset('public/serviceImage') }}/{{ $services->icon }}" data-value="{{$services->id}}">
								</div>
							</div>
						</div>
...
$.ajax({
	url:"{{url('images-delete')}}",
	type:'post',
	data:{'id':"{{$services->id}}","_token":"{{csrf_token()}}"},
	success:function(result){
	}
});
});
