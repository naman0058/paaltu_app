		...
		<div class="row g-4">
			<div class="col-lg-12 col-md-12 col-sm-12">
				<div class="card">
					...
					<div class="card-body">
						<form action="{{route('breed.update',$breeds->id)}}" method="post" id="form">
							@csrf
							@method('PUT')
							<div class="form-group col-md-6 mb-3">
								<label> Breed Name </label>
								<input type="text" value="{{$breeds->breed_name}}" class="form-control" name="breed_name" id="breed_id" placeholder="Breed Name" required>
							</div>
							<div class="form-group col-md-6 mb-3">
								...
								<select class="array-select form-control form-select" name="pet_category_id" placeholder="Pet Category" required>
									...
									@foreach($pets as $pet)
									<option value="{{$pet->id}}" @if($pet->id == $breeds->pet_category_id) selected @endif>{{$pet->name}}
									</option>
									@endforeach()
								</select>
							</div>
